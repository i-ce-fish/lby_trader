...
def WINNER(data, price_series=None):
    """
    计算获利盘比例
    data: DataFrame，包含high,low,close,volume,amount列
    price_series: Series或None，用于计算获利盘的价格序列，None时使用close
    返回: 获利盘比例序列
    """
    def calc_cost_distribution(price, volume, high, low):
        """
        计算单日筹码分布
        price: 成交均价
        volume: 成交量
        ...
        low: 最低价
        """
        min_price = np.floor(low * 100) / 100
        max_price = np.ceil(high * 100) / 100
        prices = np.arange(min_price, max_price + 0.01, 0.01)
        ...
        for p in prices:
            if p <= avg_price:
                ratio = (p - min_price) / (avg_price - min_price) if avg_price > min_price else 0
            else:
                ratio = (max_price - p) / (max_price - avg_price) if max_price > avg_price else 0
            vol = volume * ratio
            if vol > 0:  # 只记录正的成交量
                distribution[p] = vol / len(prices)
        
        return distribution

    n = len(data)
    winner_ratio = np.zeros(n)
    cost_dist = {}
    decay = 0.1  # 历史衰减系数
    
    # 使用收盘价或指定价格序列
    price_series = data.close if price_series is None else price_series
    
    for i in range(n):
        # 获取当日数据
        avg_price = data.amount.iloc[i] / data.volume.iloc[i]  # 当日均价
        high = data.high.iloc[i]
        low = data.low.iloc[i]
        volume = data.volume.iloc[i]
        compare_price = price_series.iloc[i]
        
        # 计算当日筹码分布
        new_dist = calc_cost_distribution(avg_price, volume, high, low)
        
        # 历史筹码衰减
        cost_dist = {k: v * (1 - decay) for k, v in cost_dist.items() if v > 1e-10}
        
        # 合并当日筹码
        for p, v in new_dist.items():
            cost_dist[p] = cost_dist.get(p, 0) + v
        
        # 计算获利盘比例
        if cost_dist:  # 确保有筹码分布
            total_vol = sum(cost_dist.values())
            if total_vol > 0:
                # 获利盘 = 当前价格以下的筹码量 / 总筹码量
                profit_vol = sum(v for p, v in cost_dist.items() if p < compare_price)
                winner_ratio[i] = profit_vol / total_vol
    
    return pd.Series(winner_ratio, index=data.index)
