...
def update_listen_stocks():
    ...
    m_filter = check_enter(end_date=end, strategy_fun=enter.check_ea)
    results = dict(filter(m_filter, stocks_data.items()))
    # 保存到数据库
    for stock in results.items():
        stock_code = stock[0][1]
        add_watch_stock(stock_code, stock[0][2], stock[0][3], '周期4')
    if len(results) > 0:
        stock_msg = ','.join(str(x[2]) for x in list(results.keys()))
        time  = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        wx_pusher.wx_push('周期4选股: '+stock_msg+ ' '+time)
