...
class MainEngine:
    ...
    def start(self):
        ...
        self._add_main_shutdown(self.event_engine.stop) # 添加事件引擎的shutdown

        self.quotation_engine.start() # 启动行情引擎 
        self._add_main_shutdown(self.quotation_engine.stop) # 添加行情引擎的shutdown

        self.clock_engine.start() # 启动时钟引擎
        self._add_main_shutdown(self.clock_engine.stop) # 添加时钟引擎的shutdown
    ...
    def load_strategy(self, names=None):
        ...
        s_folder = 'strategies' # 策略文件夹
        self._names = names # 策略名列表
        strategies = os.listdir(s_folder) # 获取策略文件列表
        strategies = filter(lambda file: file.endswith('.py') and file != '__init__.py', strategies) # 过滤非py文件
        importlib.import_module(s_folder) # 导入策略文件夹
        for strategy_file in strategies:
            self.load(self._names, strategy_file) # 加载策略
        # 如果线程没有启动，就启动策略监视线程
    ...
    def _load_strategy(self):
        while True:
            try:
                ...
                # print(f"主引擎：加载策略：{self.event_engine.queue_size}")
                time.sleep(2)
